...
async function loadDB() { try { return await fs.readJson(DB_PATH); } catch (e) { return { categories: [], songs: [] }; } }
async function saveDB(data) { await fs.writeJson(DB_PATH, data, { spaces: 2 }); }
...
io.on('connection', async (socket) => {
    ...
    socket.on('update_repertuar', async (data) => { await saveDB(data); io.emit('init_data', data); });
    socket.on('sahne_durumu_degistir', (isActive) => { currentSceneActive = isActive; io.emit('sahne_durumu_guncelle', isActive); });
    ...
    socket.on('oy_ver', (optionId) => {
        ...
        } else { socket.emit('oy_reddedildi', 'Zaten oy verdiniz!'); }
    });

    socket.on('anketi_bitir_sinyali', (winner) => { activePoll = null; votedIps.clear(); io.emit('anket_temizle', winner); });
});
